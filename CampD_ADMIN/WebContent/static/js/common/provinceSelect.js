 ...
 * @param areaVal
 */
ProvinceSelect.init = function(provinceEl, cityEl, areaEl,provinceVal,cityVal,areaVal) {
	var p = dsy.Items['0'];
	var options = '<option value="">请选择</option>';
	for (var i = 0; i < p.length; i++) {
		options += '<option value="' + p[i] + '" '+(provinceVal&&provinceVal==p[i]?'selected':'')+'>' + p[i] + '</option>'
	}
	provinceEl.append(options);

	provinceEl.bind('change.city', function() {
		ProvinceSelect._provinceChaged(provinceEl, cityEl, areaEl,cityVal,areaVal);
	});

	cityEl.bind('change.city', function() {
		ProvinceSelect._cityChaged(provinceEl, cityEl, areaEl,areaVal);
	});
	ProvinceSelect._provinceChaged(provinceEl, cityEl, areaEl,cityVal,areaVal);
}
ProvinceSelect._provinceChaged = function(provinceEl, cityEl, areaEl,cityVal,areaVal) {
	cityEl.empty();
	areaEl.empty();
	var options = '<option value="">请选择</option>';
	var pindex = provinceEl.find('option').filter(':selected').index()-1;
	if(pindex<0){
		cityEl.html(options);
		areaEl.html(options);
		return;
	}
	var citys = dsy.Items['0_' + pindex];
	for (var i = 0; i < citys.length; i++) {
		options += '<option value="' + citys[i] + '" '+(cityVal&&cityVal==citys[i]?'selected':'')+'>' + citys[i] + '</option>';
	}
	cityEl.html(options);
	ProvinceSelect._cityChaged(provinceEl, cityEl, areaEl,areaVal);
};

ProvinceSelect._cityChaged = function(provinceEl, cityEl, areaEl,areaVal) {
	areaEl.empty();
	var options = '<option value="">请选择</option>';
	var pindex = provinceEl.find('option').filter(':selected').index()-1;
	var cindex = cityEl.find('option').filter(':selected').index()-1;
	if(cindex<0 || pindex<0){
		areaEl.html(options);
		return;
	}
	var areas = dsy.Items['0_' + pindex + '_' + cindex]||[];
	for (var i = 0; i < areas.length; i++) {
		options += '<option value="' + areas[i] + '" '+(areaVal&&areaVal==areas[i]?'selected':'')+'>' + areas[i] + '</option>';
	}
	areaEl.html(options);
};
