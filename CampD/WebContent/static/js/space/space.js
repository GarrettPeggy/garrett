	...
	ajaxSearch(BASE_PATH + "/space/getSpaceListByParam.do",params,function(json){
		...
		if(null!=spaceList && spaceList.length>0){
			for(var i=0;i<spaceList.length;i++){
				...
				var province = spaceList[i].province;
				var city = spaceList[i].city;
				...
				space_html.push("<a href='"+BASE_PATH+"/space/getSpaceInfoById.do?id="+spaceList[i].id+"'><li class='pd5'><img src='"+OSS_RES_URL+spaceList[i].show_images.split(",")[0]+"' width='100%' height='116'/><div class='classify-li-title'>"+(null==spaceList[i].name ? "无名称" : spaceList[i].name )+"</div><div class='classify-li-desc color94 fontSize14'>"+description+"</div><div class='classify-li-date fontSize14'><img src='"+REMOTE_RES_PATH+"/static/images/date_icon.png' width='10' height='10'/>&nbsp;<span>" + province + city + area + spaceList[i].adress + "</span>&nbsp;&nbsp;<span>"+spaceList[i].traffic+"</span>&nbsp;&nbsp;<span>"+spaceList[i].cost+"元/小时</span></div></li></a>");
			}
		}else{
	...
	ajaxSearch(BASE_PATH + "/space/getSpaceListByParam.do",params,function(json){
		...
		if(null != resultList && resultList.length > 0){
			for(var i=0;i<resultList.length;i++){
				...
				$("#space_highlevel").append("<a href='"+BASE_PATH+"/space/getSpaceInfoById.do?id="+resultList[i].id+"'><li class='clearfix'><div class='data-li-left'><img src='"+OSS_RES_URL+resultList[i].show_images.split(",")[0]+"' width='91' height='63'/></div><div class='data-li-right'><div class='dlr-title' id='space_type'>"+Space.spaceType[resultList[i].space_type]+"</div><div class='dlr-address'>" + province + city + area + adress + "</div><div class='dlr-cost clearfix'><div class='fl'>费用：<span class='co'>"+resultList[i].cost+"元/小时</span></div><div class='fr'><span class='co'>"+resultList[i].capacity+"</span>人</div></div></div></li></a>");
			};
		}else{
